...
<rss version="2.0">
  <channel>
    <title>${_('Version history of activity #%s' % versions[0]['id'].split('-')[0].upper())}</title>
    <link></link>
        ...
        <description>
          <![CDATA[
            ...
            % if is_moderator and v.get('status_id') == 1:
              <span>
                <a href="${request.route_url('activities_read_one', output='review', uid=v['id'], _query=(('new', v['version']),))}">
            ...
            % if v.get('status_id') != 2:
              <span>
                <a href="${request.route_url('activities_read_one', output='compare', uid=v['id'], _query=(('ref', v['version']),('new', active_version)))}">
                  ${_('Compare this version with the active version')}
                </a>
              </span><br/>
            % endif
            <span>
              <a href="${request.route_url('activities_read_one', output='html', uid=v['id'], _query=(('v', v['version']),))}">
                ${_('View this version')}
              </a>
            </span>
          ]]>
        </description>
        <link>${request.route_url('activities_read_one', output='html', uid=v['id'], _query=(('v', v['version']),))}</link>
